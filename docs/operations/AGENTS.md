...
Only the 7 standard key files (IDENTITY.md, SOUL.md, USER.md, AGENTS.md, TOOLS.md, HEARTBEAT.md, MEMORY.md) live at the workspace root. Everything else goes into subfolders:

- `docs/` — reference materials, learnings, errors, feature requests (loaded on demand)
- `memory/` — daily session logs (`YYYY-MM-DD.md`) and heartbeat-state.json
- `drafts/` — work in progress (emails, memos, reports)
- `exports/` — completed deliverables ready for sharing
- `config/` — model routing and other config files

No loose files at root. If you create something that isn't a key file, put it in the right subfolder. For the full layout and maintenance rules, see `docs/infrastructure/WORKSPACE_STRUCTURE.md`.
...
- Ban inflated significance: "stands as a testament", "serves as a testament", "pivotal moment", "setting the stage"
...
Consider a subagent when a task would otherwise block the main chat for more than a few seconds. This keeps the conversation responsive so the user can keep talking while work happens in the background. For simple tasks or single-step operations, work directly. See `docs/operations/SUBAGENT-POLICY.md` for the full policy.
...
Convert all displayed times to the user's timezone (configured in `docs/identity/USER.md`). This includes timestamps from cron logs (stored in UTC), calendar events, email timestamps, and any other time references.
...
Skills provide your tools. Check each skill's SKILL.md for usage instructions. Keep environment-specific notes (channel IDs, paths, tokens) in `docs/operations/TOOLS.md`.
...
Follow `docs/operations/HEARTBEAT.md`. Track checks in `memory/heartbeat-state.json`. During heartbeats, commit and push uncommitted workspace changes and periodically synthesize daily notes into MEMORY.md.
...
For detailed security architecture and policies, see `docs/security/`. For multi-agent coordination, see `docs/infrastructure/KEY_FILES_MANAGEMENT.md`.
