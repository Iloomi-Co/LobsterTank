...
for ti in $(seq 0 $((TARGET_COUNT - 1))); do
    ...
    # For display purposes, keep the short path
    ...
    if [[ ! -f "$TARGET_FILE" ]]; then
        if [[ "$FORMAT" == "text" ]]; then
            echo "WARNING: $DISPLAY_PATH does not exist"
        fi
        RESULTS+=("{\"workspace\":\"$WORKSPACE\",\"agent\":\"$AGENT_NAME\",\"file\":\"$DISPLAY_PATH\",\"status\":\"missing_file\"}")
        continue
    fi
    ...
    # Build JSON result for this workspace (handle empty arrays correctly)
    if [[ ${#WORKSPACE_OK[@]} -eq 0 ]]; then OK_JSON="[]"; else OK_JSON=$(printf '%s\n' "${WORKSPACE_OK[@]}" | jq -R . | jq -s .); fi
    ...
    if [[ "$MODE" == "apply" ]] && [[ ${#WORKSPACE_MISSING[@]} -gt 0 || ${#WORKSPACE_OUTDATED[@]} -gt 0 ]]; then
        ...
        for RULE_ID in "${WORKSPACE_MISSING[@]+"${WORKSPACE_MISSING[@]}"}"; do
            ...
            # Uses Python instead of sed — BSD sed on macOS can't handle multi-line insertions
...
if idx != -1:
    sys.stdout.write(content[:idx] + insertion + '\n\n' + content[idx:])
else:
    sys.stdout.write(content.rstrip('\n') + '\n\n' + insertion + '\n')
" "$RULE_CONTENT" <<< "$UPDATED_CONTENT")

            log "ADDED: $RULE_TITLE to $DISPLAY_PATH"
            if [[ "$FORMAT" == "text" ]]; then
