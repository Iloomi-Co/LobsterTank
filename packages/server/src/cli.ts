...
async function init() {
  console.log("Poseidon — OpenClaw Control Plane");
  console.log("=============================================");
  ...
  if (!existsSync(OC_HOME)) {
    ...
    console.warn("Poseidon will start but discovery will be limited.");
...
switch (command) {
  ...
  default:
    ...
    console.log("Usage: poseidon [start|check|version]");
    process.exit(1);
}
